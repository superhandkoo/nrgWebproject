 ...
 * @returns User
 */
function getLoginTUser(){
	var user = null;
	//alert("123");
	$.ajax({
		url:baselocation+'/getloginTUser',
		type:'post',
		async:false,
		dataType:'json',
		success:function(result){
...
function initKindEditor_addblog(id, width, height,param,pressText) {
	EditorObject = KindEditor.create('textarea[id=' + id + ']', {
		...
		minWidth : '10px',
		minHeight : '10px',
		uploadJson : keuploadSimpleUrl+'&param='+param+'&fileType=jpg,gif,png,jpeg,JPG,GIF,PNG,JPEG&pressText='+pressText,// 图片上传路径
		afterBlur : function() {
			this.sync();
		},
...
//ueeditor,umeditor编辑器不可编辑的方法
function setDisabled(um) {
...
function disableBtn(str) {
    ...
    var btns = um.domUtils.getElementsByTagName(div, "button");
...
function initSimpleImageUpload(btnId,param,callback,pressText){
	KindEditor.ready(function(K) {
		var uploadbutton = K.uploadbutton({
			...
			url : uploadSimpleUrl+'&param='+param+'&fileType=jpg,gif,png,jpeg,JPG,GIF,PNG,JPEG&pressText='+pressText,
...
function webImageUpload(btnId,param,callback){
		var uploadbutton = KindEditor.uploadbutton({
			...
			fieldName : "uploadfile",
			url : uploadSimpleUrl+'&param='+param+'&fileType=jpg,gif,png,jpeg,JPG,GIF,PNG,JPEG',
			afterUpload : function(data) {
				if (data.error ==0) {
