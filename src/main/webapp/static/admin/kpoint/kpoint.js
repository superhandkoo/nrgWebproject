...
function initUpdateKpoint(treeId, treeNode){
	...
	$.ajax({
		...
		success:function(result){
			...
			$("input[name='courseKpoint.everyMoney']").val(obj.everyMoney);
			...
			$("input[name='courseKpoint.beginTime']").val(obj.beginTime1);
			$("input[name='courseKpoint.endTime']").val(obj.endTime1);
			...
			if(obj.fileType=='VIDEO'){//选择视频格式
				$(".videoType").show();
			}else if(obj.fileType=='TXT'){//选择文本格式
				$(".txtContent").show();
			}
			$("#updateWin").show();
...
function updateKpoint(){
	...
	var money=	$("input[name='courseKpoint.everyMoney']").val();
	if(!reg.test(money)){
		alert('价格必须是正整数！');
		return false;
	}
	var begin=$("input[name='courseKpoint.beginTime']").val();
	if(begin==null||$.trim(begin)==''){
		alert("请填写课时开始时间");
		return false;
	}
	var end=$("input[name='courseKpoint.endTime']").val();
	...
	var teacher=$('input[name="courseKpoint.teacherId"]').val();
	if(teacher==null || $.trim(teacher)==''||teacher==0){
		alert("老师不能为空");
		return false;
	}
...
function deleteKpoint(treeId, treeNode){
	...
	$.ajax({
		url:baselocation+'/admin/kpoint/deletekpoint/'+treeNode.kpointId,
		type:'post',
		dataType:'json',
