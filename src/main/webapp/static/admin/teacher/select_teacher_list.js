...
function selectTeacher(type){
	...
	var id=$("#returnIds").val();
	if(id==""||id==undefined||id==null){
		id=0;
	}
	window.open(baselocation+'/admin/teacher/selectlist/'+type+'/'+id ,'newwindow', 'toolbar=no,scrollbars=yes,location=no,resizable=no,top=150,left=300,width=923,height=600');
}

function selectAssistant1(type){
	if(type==null || $.trim(type)==''){
		type='checkbox';
	}
	window.open(baselocation+'/admin/teacher/selectAssistantlist/'+type ,'newwindow', 'toolbar=no,scrollbars=yes,location=no,resizable=no,top=150,left=300,width=923,height=600');
}
...
function confirmSelect2(){
	...
	window.opener.addAssistantList(teahcerArr);
	closeWin();
}
