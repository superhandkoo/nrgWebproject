...
function addAssistantList(tcArr){
	var pList = $("#assistantList p");
	var ids = $("input[name='assistantIdArr']").val();
	var tcHtml='';
	var tcIds='';
	for(var i=0;i<tcArr.length;i++){
		if(pList!=null && pList.length>0){
		...
		tcIds+=tcArr[i].id+',';
		tcHtml+='<p id="'+tcArr[i].id+'" style="margin: 0 0 0em;">'+tcArr[i].name+'&nbsp;&nbsp;<a href="javascript:void(0)" onclick="deleteTc2('+tcArr[i].id+')">删除</a></p>';
	}
	$("input[name='assistantIdArr']").val(tcIds+ids);
	$("#assistantList").append(tcHtml);
}
/**
 * 选择讲师后的回调 
 * @param tcArr 讲师数组
...
function deleteTc2(tcId){
	$("#assistantList #"+tcId).remove();
	var ids = $("input[name='assistantIdArr']").val();
	ids= ids.replace(tcId+',','');
	$("input[name='assistantIdArr']").val(ids);
}

/**
 * 保存课程
 */
function saveCourse(flag){

	var subjectId = $("input[name='course.subjectId']").val();
	var gradeFlag = $("input[name='course.subjectLink']").val();
	$("#levelId").find("select").eq(1).attr("id","gradeName");
	var gradeName = $("#gradeName option:selected").text();
	if(subjectId<=0){
		alert("请选择学校/年级");
		return false;
	}
	if(gradeName==""||gradeName==undefined||gradeName=="请选择"){
		alert("请选择年级");
		return;
	}
	...
	if(flag==1){
		...
		if(teacherIdArr==null||teacherIdArr==undefined||teacherIdArr==""){
			alert("请选择老师");
			return;
		}
	}
	...
	var orginName=$("input[name='orginName']").val();
	var url=basepath+"/admin/cou/checkCourseName";
	if(orginName!=undefined||orginName!=''||orginName!=null){
		//是修改
		if(orginName==courseName){
			//不进行课程名重复查询
			...
			$.ajax({
				url:url,
				type:"post",
				data:{"courseName":courseName,"gradeId":subjectId},
				dataType:"json",
				async:false,
